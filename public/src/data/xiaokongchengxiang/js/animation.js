...
// 弹出效果
function popup(element, duration = 500) {
    if (!element) return;
    ...
    function animate(timestamp) {
        ...
        const easeOutProgress = 1 - Math.pow(1 - percent, 3); // 缓出效果
        
        element.style.opacity = easeOutProgress;
        
        // 弹出效果，稍微超过然后回弹
        let scale;
        if (percent < 0.7) {
            scale = 0.8 + (0.3 * easeOutProgress);
        } else {
            scale = 1.1 - (0.1 * ((percent - 0.7) / 0.3));
        }
...
// 闪烁效果
function pulse(element, times = 3, duration = 1200) {
    if (!element) return;
    
    const initialBgColor = window.getComputedStyle(element).backgroundColor;
    const initialBorderColor = window.getComputedStyle(element).borderColor;
    const pulseColor = 'rgba(255, 210, 0, 0.6)';
    const pulseBorderColor = 'rgba(255, 180, 0, 1)';
    ...
    function animate(timestamp) {
        if (!start) start = timestamp;
        const progress = (timestamp - start) % (duration / times);
        const percent = progress / (duration / times);
        
        if (percent < 0.5) {
            // 淡入高亮色
            const factor = percent * 2;
            element.style.backgroundColor = pulseColor;
            element.style.borderColor = pulseBorderColor;
            element.style.boxShadow = `0 0 ${15 * factor}px rgba(255, 180, 0, ${0.8 * factor})`;
        } else {
            // 淡出恢复原色
            const factor = (1 - percent) * 2;
            element.style.backgroundColor = initialBgColor;
            element.style.borderColor = initialBorderColor;
            element.style.boxShadow = `0 0 ${15 * factor}px rgba(255, 180, 0, ${0.8 * factor})`;
        }
